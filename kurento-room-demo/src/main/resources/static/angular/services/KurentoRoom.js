...
function Room(kurento, options) {
    ...
    this.connect = function () {
        ...
        if (localParticipant) {
            var streams = localParticipant.getStreams();
            if (Object.keys(streams).some(function (streamId) {
    ...
    this.onParticipantPublished = function (options) {
        console.debug("onParticipantPublished!");
    ...
    this.recvIceCandidate = function (msg) {
        ...
        //console.debug("receive ICE candidate endpoint " + msg.endpointName + " streamId " + msg.streamId +" for "+participant.getID()+" "+stream.getID());
    ...
    this.disconnect = function (stream) {
        var participant = stream.getParticipant();
        if (!participant) {
            console.error("Stream to unpublish has no participant ", partid);
            return false;
        }

        participant.removeStream(stream.getID());
        ...
        if (participant === localParticipant) {
            ...
            console.log("Unsubscribing from " + stream.getGlobalID());
            kurento.sendRequest('unsubscribeFromVideo', {
                sender: participant.getID,   //stream.getGlobalID(),
                streamId: stream.getID()
            },
                function (error, response) {
                    if (error) {
...
function Participant(kurento, local, room, options) {
    ...
    if (options.streams) {
        for (var i = 0; i < options.streams.length; i++) {
            console.debug("new remote stream " + options.id + " " + options.streams[i].streamId);
            var streamOpts = {
...
function Stream(kurento, local, room, options) {
    ...
    this.init = function () {
        ...
        //getScreenMedia(constraints, function (userStream) {
        getMedia(constraints, function (userStream) {
            wrStream = userStream;
    ...
    this.startVideoCallback = function (error, sdpOfferParam, wp) {
        ...
        console.log("Sending SDP offer to subscribe to "
            + participant.getID() + " "
            + that.getID(), sdpOfferParam);
        kurento.sendRequest("receiveVideoFrom", {
            sender: participant.getID(),  //that.getGlobalID(),
            streamId: that.getID(),
            sdpOffer: sdpOfferParam
        }, function (error, response) {
            if (error) {
    ...
    function initWebRtcPeer(sdpOfferCallback) {
        if (local) {
            var options = {
                ...
                onicecandidate: that.sendIceCandidate.bind(that), 
    ...
    this.processSdpAnswer = function (sdpAnswer) {
        ...
        pc.setRemoteDescription(answer, function () {
            ...
            if (!local || that.displayMyRemote()) {
                ...
                console.log("Peer remote stream " + participantId, wrStream);
    ...
    this.sendIceCandidate = function (candidate) {
        console.debug((local ? "Local" : "Remote"), "candidate for",
            participant.getID(), id, JSON.stringify(candidate));
        kurento.sendRequest("onIceCandidate", {
            endpointName: participant.getID(),
            streamId: that.getID(),
            candidate: candidate.candidate,
            sdpMid: candidate.sdpMid,
            sdpMLineIndex: candidate.sdpMLineIndex
